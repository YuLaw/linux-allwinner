...
static int ctp_set_gpio_mode(void)
{
	...
	gpio_int_hdle = gpio_request_ex("ctp3_para", "ctp_io_port");
	if(!gpio_int_hdle){
...
static int ctp_init_platform_resource(void)
{
	...
	gpio_wakeup_hdle = gpio_request_ex("ctp3_para", "ctp_wakeup");
	if(!gpio_wakeup_hdle) {
		pr_warning("%s: tp_wakeup request gpio fail!\n", __func__);
		gpio_wakeup_enable = 0;
	}

	gpio_reset_hdle = gpio_request_ex("ctp3_para", "ctp_reset");
	if(!gpio_reset_hdle) {
...
static int ctp_fetch_sysconfig_para(void)
{
	...
	if(SCRIPT_PARSER_OK != script_parser_fetch("ctp3_para", "ctp_used", &ctp_used, 1)){
	...
	if(SCRIPT_PARSER_OK != script_parser_fetch_ex("ctp3_para", "ctp_name", (int *)(&name), &type, sizeof(name)/sizeof(int))){
		pr_err("%s: script_parser_fetch err. \n", __func__);
		goto script_parser_fetch_err;
	}
	...
	if(SCRIPT_PARSER_OK != script_parser_fetch("ctp3_para", "ctp_twi_addr", &twi_addr, sizeof(twi_addr)/sizeof(__u32))){
		pr_err("%s: script_parser_fetch err. \n", name);
		goto script_parser_fetch_err;
	}
	...
	if(SCRIPT_PARSER_OK != script_parser_fetch("ctp3_para", "ctp_twi_id", &twi_id, sizeof(twi_id)/sizeof(__u32))){
		pr_err("%s: script_parser_fetch err. \n", name);
		goto script_parser_fetch_err;
	}
	pr_info("%s: ctp_twi_id is %d. \n", __func__, twi_id);

	if(SCRIPT_PARSER_OK != script_parser_fetch("ctp3_para", "ctp_screen_max_x", &screen_max_x, 1)){
		pr_err("%s: script_parser_fetch err. \n", __func__);
		goto script_parser_fetch_err;
	}
	pr_info("%s: screen_max_x = %d. \n", __func__, screen_max_x);

	if(SCRIPT_PARSER_OK != script_parser_fetch("ctp3_para", "ctp_screen_max_y", &screen_max_y, 1)){
		pr_err("%s: script_parser_fetch err. \n", __func__);
		goto script_parser_fetch_err;
	}
	pr_info("%s: screen_max_y = %d. \n", __func__, screen_max_y);

	if(SCRIPT_PARSER_OK != script_parser_fetch("ctp3_para", "ctp_revert_x_flag", &revert_x_flag, 1)){
		pr_err("%s: script_parser_fetch err. \n", __func__);
		goto script_parser_fetch_err;
	}
	pr_info("%s: revert_x_flag = %d. \n", __func__, revert_x_flag);

	if(SCRIPT_PARSER_OK != script_parser_fetch("ctp3_para", "ctp_revert_y_flag", &revert_y_flag, 1)){
		pr_err("%s: script_parser_fetch err. \n", __func__);
		goto script_parser_fetch_err;
	}
	pr_info("%s: revert_y_flag = %d. \n", __func__, revert_y_flag);

	if(SCRIPT_PARSER_OK != script_parser_fetch("ctp3_para", "ctp_exchange_x_y_flag", &exchange_x_y_flag, 1)){
...
#ifdef TOUCH_KEY_LIGHT_SUPPORT
	gpio_light_hdle = gpio_request_ex("ctp3_para", "ctp_light");
#endif
...
//	pr_info("==INIT_WORK=\n");
	...
	err = ctp_ops.set_irq_mode("ctp3_para", "ctp_int_port", CTP_IRQ_NO, CTP_IRQ_MODE);
	if(0 != err){
